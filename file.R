...
shp <- read_sf('Local_Authority_Districts_(December_2011)_Boundaries_EW_BFC.shp')

#shp0 = shp
##shp = subset(shp, str_detect("W", lad11cd))
shp <- fortify(shp, region = 'lad11cd')

#shp = subset(shp, substr(id,1,1)!="W")
...
#dataURL = "https://raw.githubusercontent.com/nujcharee/DSDA_Data/main/ukpopestimatesmid2020on2021geography.xls"
#download.file(dataURL, destfile=temp, mode='wb')

#pop <- readxl::read_excel(temp, sheet ="MYE2 - Persons", skip = 6)

#make first row column names
#pop = janitor::row_to_names(pop,row_number = 1)
